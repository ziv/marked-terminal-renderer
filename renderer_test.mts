...
test('mixed inline styles', async () => {
  const result = await marked.parse('**bold** and *italic* and ~~strikethrough~~');
  const stripped = stripAnsi(result);
...
test('link with title', async () => {
  const result = await marked.parse('[GitHub](https://github.com "GitHub Homepage")');
  const stripped = stripAnsi(result);
...
test('paragraph text wrapping', async () => {
  const longText = 'This is a very long line of text that should be wrapped to fit within the terminal width limit that has been configured for the renderer instance.';
...
test('symbol replacements', async () => {
  const result = await marked.parse('Copyright (c) Trademark (tm) Registered (r)');
  const stripped = stripAnsi(result);
